...
def get_mch_heatmap(ensemble, methylation_type, grouping, clustering, level, ptile_start, ptile_end, normalize_row, query):
    ...
    layout = Layout(
        # autosize=True,
        # height=550,
        height=max(550*len(genes)/20,300), # EAM Adjust the height of the heatmap according to the number of genes displayed
        width=1000,
        # title=title,
        # titlefont={'color': 'rgba(1,2,2,1)',
        #            'size': 16},
...
@cache.memoize(timeout=3600)
def get_mch_clusterbar(ensemble, methylation_type, grouping, clustering, level):
    """Generate cluster bar plot
    ...
    # TODO: Query mysql to get a bar plot of the number of cells per cluster....
